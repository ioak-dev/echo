...
const ProtectedRouteApp = (props: Props) => {
  ...
  const authenticate = async (type: string, redirect = true) => {
    ...
    const accessToken = getSessionValue(`echo-access_token`);
    const refreshToken = getSessionValue(`echo-refresh_token`);
        ...
        .then((response: any) => {
          if (response.status === 200) {
            ...
            if (response.data?.access_token) {
              ...
              setSessionValue(`echo-access_token`, newAccessToken);
            }
            dispatch(
              addAuth({
                isAuth: true,
